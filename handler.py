    ...
    try:
        ...
        if "prompt" in input_data:
            return {
                ...
                "capabilities": ["마스킹 감지", "웨딩링 보정", "업스케일링", "썸네일 생성"],
                "version": "v13.3",
                "params_loaded": len(WEDDING_RING_PARAMS)
            }
        ...
        if "image_base64" in input_data:
            ...
            # 3. 웨딩링 영역만 추출하여 보정
            ring_mask_inv = 255 - mask
            masked_image = image_array.copy()
            masked_image[ring_mask_inv > 0] = 0
            
            # 4. v13.3 웨딩링 보정
            enhanced_ring = processor.enhance_wedding_ring(masked_image, metal_type, lighting)
            
            # 5. 2x 업스케일링
            upscaled = processor.basic_upscale(enhanced_ring, scale=2)
            ...
            return {
                ...
                "processing_info": {
                    ...
                    "masking_area": f"{bbox[2]}x{bbox[3]}",
                    "version": "v13.3"
                }
            }
